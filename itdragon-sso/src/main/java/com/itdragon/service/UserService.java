...
public class UserService {
    ...
    public int logout(String token, HttpServletRequest request, HttpServletResponse response) {
    	Long ret = jedisClient.del(REDIS_USER_SESSION_KEY + ":" + token);
    	// 删除cookie
        System.out.println("客户端退出登录..." + String.valueOf(ret));
		if(ret > 0) {
			CookieUtils.deleteCookie(request, response, "USER_TOKEN");
			return 1;
		}
		// TODO 回调所有SSO 客户端，调用客户端注销接口，销毁cookie.
        return 0;
    }
